...
<!-- STAMP -->
> **Generated 2026-08-08 17:55, on top of commit `abe1739`.**
>
...
> checked.
>
> **Cross-check:** commit `abe1739` must appear in this repo's commit
...
<!-- SECTION:coordinator updated=2026-08-08T17:55 -->
## How to read this page

**As of 2026-08-08.** This replaced six separate `BRIEF_*.md` files, which were
drifting into a pile where working out the current one was itself a job. Now
there is one page, one section per project. Each session overwrites **only its
own section**, through a command that physically cannot reach another section.

### The connection test passed — and killed two of my own claims

A marker word was pushed into this page on 2026-08-08 and the coordinating chat
was asked what it saw.

- **✅ This address works.** Found first try, fresh, correct timestamp.
- **❌ The repo-root `BRIEF.md` is frozen forever for that reader.** The marker
  was **not** there. It cached on first fetch and never updates. **That address
  is never handed out.**
- **❌ The pages cannot be walked automatically.** An earlier version of this
  box told the reader to follow a link to the next page. **It cannot** — an
  address printed inside a plain-text file is not a link that fetcher follows.
  **Retracted.**

**So the claim that this would need no copy-paste is withdrawn.** Three attempts
on three different mechanisms all failed. **One paste per page is the floor and
is now accepted rather than engineered around.**

**What replaced it:** every session ends every message with the current address,
ready to copy. The paste stays; hunting for what to paste is gone.

### What makes this page safe to trust

**It never changes.** Every version is written to its own permanent address and
never edited afterwards, so what you are reading is exactly what was true at the
timestamp above. A page that cannot change cannot go quietly stale — that is the
one property doing real work here.

### Two things this page can tell you that GitHub cannot

Because a session reading the repo sits on the actual machine:

- whether work exists on disk that has **not been pushed** — you are then
  reading an older picture, and it will be named;
- whether a project **did work it has not written up** — its last commit newer
  than its section here.

### What this page is not

It reports state, not truth. Every claim below was written by the session that
did the work and is that session's responsibility. Nothing here has been
re-audited. Claims get tested in `LEDGER.md` and `GUARDS.md`, unchanged.

**The coordinator holds no credential, makes no network call of any kind, and
cannot place a trade.** A test fails the moment any of that changes.

Design and the full list of what it cannot do:
[coordinator/COORDINATOR.md](coordinator/COORDINATOR.md).

_Section `coordinator` last written 2026-08-08 17:55._
<!-- /SECTION:coordinator -->
