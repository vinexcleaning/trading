...
<!-- STAMP -->
> **Generated 2026-08-08 14:39, on top of commit `df5bd94`.**
>
> **Automated readers: follow the chain below. Do not re-fetch this
> page and do not add a query string.** Query strings are discarded by
> at least one fetcher in use here: two requests differing only in a `v=`
> parameter returned the same cached body. Only a different **path** is
> fetched fresh.
>
> **Next page, if one exists:**
> `https://raw.githubusercontent.com/vinexcleaning/trading/main/briefs/BRIEF-2026-08-08-02.md`
> Fetch it. If it loads, read it and follow *its* next link. Repeat
> until one returns 404 — the last page that loaded is the newest.
>
> **To skip a whole day forward:**
> `https://raw.githubusercontent.com/vinexcleaning/trading/main/briefs/BRIEF-2026-08-09.md`
> One page exists per day the repo was worked on, holding that day's
> final state. A 404 means nothing was published that day — try the
> day after, and so on up to today's date.
>
> **Cross-check:** commit `df5bd94` must appear in this repo's commit
> history. If it does not, this page predates it and is cached.
<!-- /STAMP -->

---

<!-- SECTION:coordinator updated=2026-08-08T14:39 -->
CONNECTION TEST 2026-08-08 PELICAN
...
### If you are an automated reader, follow the links in the box above

Every version of this page also lives at its own permanent path under `briefs/`,
and **each page names the path of the next one**. Follow next-links until one
returns 404 — the last page that loaded is the newest.

**Do not add a query string to the address.** It is discarded: two requests
differing only in a `v=` parameter came back with the same cached body. That was
tested on 2026-08-07 after an earlier version of this page claimed the opposite.
**That claim was wrong and is retracted.** Only a different path is fetched
fresh, which is the entire reason `briefs/` exists.

### Two things this page can tell you that GitHub cannot

Because a session reading it is sitting on the actual machine:
...
re-audited by the coordinator. Claims get tested in `LEDGER.md` and `GUARDS.md`,
and that has not changed.

### Instructions travel through files, not copy-paste

Give an instruction to the coordinator session once; it files it to whichever
sessions need it, and each replies in the same file. The honest limit: the
coordinator can *leave* a message, it cannot *deliver* one. A session reads its
mail when it next starts, or when you say "check your mail" in that window. A
session already mid-task will not see it until it finishes.
...
_Section `coordinator` last written 2026-08-08 14:39._
<!-- /SECTION:coordinator -->
