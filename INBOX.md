...
- 2026-08-18 — Build the STRATEGY FACTORY: continuous strategy generation, screening on recorded tape, forward paper-trading the survivors. Plan: coordinator/STRATEGY_FACTORY.md. Filed to the new `factory` chat.
- 2026-08-18 — Move the strategy-factory method off Kalshi and onto real markets (crypto perps, equities, forex). His idea. NOT STARTED, plan given and awaiting his go. Argument: the recorded Kalshi cost bar is 3.61c on a 50c contract = 7.2% round trip (S004), against roughly 0.12% on a crypto perp - about 60x - and Kalshi history is a 69-day rolling window against years of free bars. Main hazard: losses are NOT capped there, which flips the stop-loss rule in CLAUDE.md 9c. Routing undecided: this repo is PUBLIC and is scoped to prediction markets.
- 2026-08-20 — Test the set-1 fade as MAKER rather than taker. His idea, reached via a fee question. The effect is +2.42c against a 3.61c bar made of 1.20 spread + 1.00 slippage + 1.44 fee; resting an order attacks all three, and set1_overshoot/PHASE5_RESULTS.md names it as a 3.636c-ceiling lever that was never pulled. Maker FAILED on crypto ladders (C022, -0.853c, CI excludes zero) - a warning, not a verdict, since tennis has different participants and time structure. Filed to tennis. Also asks whether the PRICE-drop trigger (90c to 50c) was ever run, as distinct from the set-score trigger.
