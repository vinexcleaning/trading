...
|---|---|---|---|
| **MB010** | Every `mlb-paper` entry decision before 2026-09-02 was taken against a cost bar **2 to 6 times too strict** | mlb-paper | 863 archive games; 1,432 live positions | **Two stacked errors.** Kalshi charges **half** fee on `KXMLBGAME`/`KXMLBTOTAL` (`fee_multiplier` 0.5, verified live against the API by three chats independently), and the gate additionally applied the per-**order** round-up to a **single contract**. At a 52c ask it subtracted **2.000c** where the real cost is **0.874c**; at 95c the ratio is **6x**. **The error was in the SAFE direction — it suppressed bets, it never manufactured them — so results recorded before this date are understated, not inflated.** Re-running the archive with the corrected bar admits **67% more bets** (`starter` 298 → 498) and moves the overall return only to **+0.6% on 498 games**. Recorded fees overstate by **$154.82 / 1,432 positions**; the book is deliberately not re-priced. ⚠ **Undateable:** the multiplier is known 0.5 on 2026-08-18 and today; before that is unrecoverable. |
...
| S018 | Label coverage cannot be raised. Apify hard-capped; Flashscore `dayOffsets` is −7..+7 against a −68 need. | set1_overshoot | Apify error payload; actor schema | — | 08-01 | structural blocker; true cost ~$20 not $3.44 | n/a | n/a | **SETTLED** — ⚠ **REFUTED, not resolved, and now HALF-ANSWERED.** My own per-season workbooks took the label count **479 → 1,062** and the smallest visible effect from ~9.9 to ~6.6 against a **3.61** cost bar — **still does not clear.** Main tour is solved from free sources; **the ITF half is not, and that is the half that matters, because the Kalshi pool is 73–87% ITF.** The exact remaining ask, if a source is ever proposed: *Challenger and ITF match results with per-set games won, 2023 to within a week of today, updated weekly* — and **open the file before believing its description** (GUARDS #25: `M016b` called itself live and its newest match was 2026-01-17). Marked 2026-08-21 per `tennis` mailbox 018 |
...
| C016 | ⚠ **WORDING NARROWED 2026-08-09.** Reads as a general property of cheap wings; it is **one event's final hour** (a 60-strike ladder, per-minute). `MORNING_REPORT.md` §0000 already carries the *"Refinement, so this is not overstated"* paragraph confining it to the far wings, and the conclusion is reached independently by the C6 withdrawal — so this is a sentence fix, not a re-run. Original: The "cheap wings" are not tradeable — they have an ask but **no bid**. | crypto | `reports/b1_KXBTCD.json`, `MORNING_REPORT.md` §0000 | 60-strike ladder, per-minute over final hour | 08-01 | at \|K−settle\| ≥ $893: **0 of 61 minutes** two-sided, all 11 strikes | n/a | n/a | **SETTLED** |
...
| T002 | The binding constraint: Sackmann features end **2026-06-02**, and 85.0% of markets are after that. | kalshi-tennis | same | 20,922 markets | to 2026-07 | only **3,145** markets are both settled and inside the window | n/a | n/a | **SETTLED** — ⚠ **needs paid tennis history, which the user DECLINED on 2026-08-20 ("I don't wanna pay any money for tennis history"), second time of asking. CLOSED — do not raise again.** Open and unanswerable from free sources, not merely unfinished. Marked 2026-08-21 per `tennis` mailbox 018 |
...
| **B023** | **Pre-match player features add nothing to Kalshi's opening price.** | bot-forensics | `src/t6_features.py`, `src/t7_sweep.py` → `out/t7_sweep_*.csv`, [FINDINGS_T7.md](bot-forensics/FINDINGS_T7.md) | **6,519 events** (4,563 train / 1,956 holdout), **2,008 cells** | 06-29 → 07-27 | **2 BH discoveries; the same machinery on shuffled data yields 4.1 on average.** Real max\|t\| **4.17** vs null mean **4.40** | **BH-FDR 5%, one denominator over all 2,008** | **yes, time-ordered 70/30** | **SETTLED** (null) — pre-registered before running. **A sweep that finds less than its own null has found nothing** — ⚠ **stays a 29-day null and cannot be improved from free sources.** Widening it needs paid tennis history, DECLINED 2026-08-20 and CLOSED. Marked 2026-08-21 per `tennis` mailbox 018 |
| **B024** | The one surviving signal — "buy the heavy favourite" — **is a wide-book quoting artifact, not a mispricing.** | bot-forensics | `src/t7_sweep.py`; spread-stratified table in FINDINGS_T7 | 952 events ≥80c | 06-29 → 07-27 | residual by opening spread: **≤2c → +1.18pp (t=0.64)** · 2–4c +4.87pp · 4–8c +3.50pp · **>8c +7.92pp**. Monotonic in spread; gone where tradeable | within B023's family | holdout same sign, **net at ask ~~−0.77c~~ −0.374c** ⚠ **CORRECTED 2026-09-01 to −0.374c** — the −0.770c charged `fee_order_cents(px, 1)`, the per-ORDER round-up, to orders of ONE contract in expectancy arithmetic; `common/kalshi_fees.py` names `fee_rate_cents` for exactly this case. Reproduced (n=261) then recomputed. **+0.396c of the old number was the rounding assumption, not economics.** The old figure is left visible per house rules. **B024 is unchanged** — still negative, and the cell's 6.06c mean spread is the killer, not the fee. Found by the `reopen` audit. | **SETTLED** — a mid quoted inside an 11.8c spread is not a price. Tight-book MDE **5.15pp**, so a real effect is not excluded, only unevidenced |
...
| **BH013** | ~~`RESULTS.md` §3's "`KXMLBGAME` is 1.0¢ at every lead" is wrong because the recorded touch is 2.0¢~~ ⚠⚠ **MY CORRECTION WAS ITSELF WRONG. WITHDRAWN 2026-08-09.** | `src/mlb_scope.py`, re-measured pre/post the BH014 fix | pre-fix 12,780 snapshots / 122 tickers · **post-fix 18,828 / 152** | **pre-fix median 2.0¢, mean 3.13¢, p90 7.0¢ → POST-FIX median 1.0¢, mean 1.32¢, p90 2.0¢** | **The 2.0¢/7.0¢ I used to "correct" RESULTS.md was an artifact of the starved recorder (BH014).** With even coverage the recorded touch is **1.0¢ median, 2.0¢ p90 — which is what RESULTS.md's candle reading said in the first place.** Coverage evenness confirms the mechanism: snapshots per ticker p25 went **25 → 94**. ⚠ *Stated against the tidy version:* the two samples are not identical populations — the fix deliberately orders by `close_time` ascending, so post-fix skews to soonest-closing and more liquid markets. **That is the right population for a pre-match strategy**, which is why 1.0¢ is the number to quote, but neither figure is "the" spread for all MLB markets. | **RETRACTED (my own correction). RESULTS.md was right.** No verdict moves: the cost bar is `fee(ask) + slippage` with **no half-spread term** (PREREGISTRATION_DEVIG §2.3), so it never depended on this, and a tighter book makes venue gaps smaller, not larger |
...
| **BH015** | ⚠ **R1 IS DEAD, on day one, before any game settled.** De-vig a LOOSE RETAIL book (Bovada) and compare it to the SHARP book (Pinnacle) on the same games: **they are the same opinion sold at different prices.** | `src/retail_n3.py` → `reports/retail_n3_dayone*.json`; pre-registered at `PREREGISTRATION_RETAIL.md` **before any number existed** | **11 games, twice, 2026-08-14 at 06:49Z and 13:44Z**; all three feeds pulled within **50 s and 65 s** so drift cannot manufacture the gap | Bovada margin **4.46 out of 100** vs Pinnacle **1.98** — **2.25× fatter** — yet after each book's own margin is stripped they agree to **0.18¢ median, 0.48¢ max**, against a cheapest cost of **1.61¢**. **0 of 11 clear, both runs.** ⚠ **The three de-vig methods DISAGREE IN SIGN** (proportional −0.03/−0.02¢, power +0.08/+0.12¢, Shin +0.04/+0.07¢) — §3a declared in advance that this *is* the finding. The whole method spread is **0.14¢** and the book disagreement is **0.18¢**: instrument and signal are the same size. | **SETTLED — 2 of 4 pre-registered drop conditions fired.** Scope is exact: **the baseball moneyline, Bovada, two instants on one day.** Untested and listed in `RESULTS_RETAIL_N3.md` §4: the other 466 Bovada markets, tennis (160 events), the four permitted books never parsed, intraday gaps, and the player props. **Third demonstration that a fat margin is not evidence of room** |
| **BH016** | **86 out of every 100 open Kalshi baseball markets have no free sharp reference at all.** ⚠ Not a mispricing claim — the absence of a cheap way to find out you are wrong (M024's retracted inference). | `src/blind_spots.py` → `reports/baseball_blind_spots.json` | **4,291 open markets, 110 series, 2026-08-14** — series enumerated first, because v1 paged unfiltered `/markets` at a 40-page cap, saw 8,000 markets, found no baseball and returned **zero** | 605 markets have a Pinnacle straight-market reference; **3,686 across 102 series do not**. ⚠ **And `RESEARCH`'s "no strikeout line, no first-five total" is WRONG as of today**: the free feed carries **62 two-sided player props** and **105 first-five totals**, joining to Kalshi on **10 of 20** strikeout pitchers and **13 of 21** home-run hitters | **SETTLED for the census; the prop join is UNVERIFIED and needs its own pre-registration.** Explicitly NOT covered by any de-vig result here — every one was measured on the game winner |
| **BH017** | **Kalshi's RUN TOTALS agree with the sharp book to less than one tick.** Largest family on the recorder (2,280 tickers, 96,336 snapshots since 2026-08-04) and no strategy had ever been written against it. | `src/totals_n3.py` → `reports/totals_n3.json` | **30 rungs across 9 GAMES**, 2026-08-20 04:55Z, both feeds pulled within **41 s**. ⚠ **Games, not rungs, is the count** — one ladder settles on one score | Kalshi's ask came back on a whole cent every time; median gap to the de-vigged sharp fair **0.43¢**, p90 0.84¢, **max 1.00¢**, against a median fee of **1.71¢**. **0 of 30 clear, buy side AND sell side.** All three de-vig methods negative (−0.14 / −0.07 / −0.09¢), so no sign disagreement. Pinnacle's totals margin **3.96 out of 100** against its own moneyline's **1.98** — **fourth measured demonstration that a fat margin is not evidence of room** | **SETTLED for the 30 rungs a sharp book quotes. ⚠ EXPLICITLY UNTESTED for the other 69**, which is the finding that matters — see BH018 |
| **BH018** | ⚠ **The Kalshi totals rungs that are cheap enough to trade are exactly the ones nobody can check.** | same run | 99 open rungs across the same 9 games | **30 rungs have a free sharp reference** (ask 37–68¢, fee **1.71¢** median); **69 have none** (ask 15–97¢, fee 1.12¢ median, **0.20¢ minimum**). **15 of the 69 sit at ≤10¢ or ≥90¢ where the fee is ~0.33¢ — and not one has a reference.** Cause: the sharp book quotes only the three or four lines nearest the true total; Kalshi quotes the whole ladder from ~2.5 to ~13.5 | **SETTLED as a fact about coverage. ⚠ It is NOT evidence those rungs are mispriced** — that is M024's retracted argument and it stays retracted. It is the absence of a cheap way to find out you are wrong, which makes a model the only instrument that reaches them and a wrong model there expensive to detect |
| **BH019** | **First-five-innings totals and TEAM totals are dead too.** The two families `RESULTS_TOTALS_N3.md` had just listed as untested. | `src/totals_family_n3.py` → `reports/totals_family_n3.json` | **25 rungs (F5) and 54 rungs (team) across the same 9 GAMES**, 2026-08-20, feeds 23 s and 81 s apart | F5: median gap **0.73¢**, max 1.58¢, fee 1.69¢ — **0 of 25**. Team: median **1.20¢**, max **2.79¢**, fee 1.68¢ — **0 of 54**. **109 rungs across three families, not one clears, buy or sell.** ⚠ Team totals carry **Pinnacle's fattest margin measured anywhere — 5.44 out of 100**, nearly 3× its own moneyline — and the largest gap, **and the gap points the WRONG WAY** (all three methods −0.9 to −1.1¢, i.e. Kalshi's ask sits ABOVE the sharp fair). **Fifth and cleanest demonstration that a fat margin is not evidence of room.** ⚠ 54 rungs on 18 team-games is not 54 observations nor 18 — the two teams share a game state, so **9** is the count used | **SETTLED for the referenced rungs.** Coverage repeats across all three families: **179 rungs have no free sharp price**, and every cheap-fee rung is among them (BH018) |
| **BH020** | ⚠ **THE MECHANISM: the sharp book's fair value sits INSIDE Kalshi's own bid–ask on 70 out of 100 rungs. There is no disagreement to trade; there is a spread to pay.** | `reports/totals_n3.json` + `reports/totals_family_n3.json` | **109 rungs, 3 totals families, 9 games**, 2026-08-20 | **76 of 109 (70%)** have the de-vigged sharp fair between Kalshi's bid and ask. **And the share rises exactly with the spread** — game totals 57% at a 1.0¢ spread, first-five 68% at 2.0¢, team totals **78% at 3.0¢**. ⚠ **So the family that appeared to disagree most (team totals, max 2.79¢ against a 1.68¢ fee) was showing its half-spread, not a disagreement** — buying the over cost 2.79¢ over fair AND buying the under cost 1.21¢ over fair, simultaneously, which is what a spread is. **I nearly wrote that 2.79¢ up as a tradeable gap.** | **SETTLED, and it supersedes 'the gap is too small' as the explanation for BH017 and BH019.** It also predicts where any future venue-vs-venue test will fail: wherever Kalshi's spread exceeds twice the true disagreement, which is nearly everywhere measured so far |
| **BH021** | ⚠⚠ **RETRACTED — MY OWN, 2 DAYS OLD. "The sharp book's player-prop board opens 16.9 hours before first pitch" came from ONE day and has failed to reproduce twice.** | `reports/prop_watch.jsonl`, `logs/props_n3.log`, live check | **08-18**: 47 samples, props live **44 of 45**, first seen 16.9 h out. **08-20**: 17 samples over 4 h inside that same window — **empty throughout**. **08-21**: checked at **16.1 h out**, the same point in the cycle — **empty** | All three passed the GUARDS #27 control (same call returned Exact Scores 209–228, Next Run 12, Double Result 10–12), so it is a genuinely empty prop board on all three days, not a block. **Untested hypothesis, named so it is not mistaken for a measurement: props may appear only once starting pitchers are confirmed.** | **RETRACTED.** The §3a kill condition was written against the wrong failure — the risk is not a SHORT window but an UNPREDICTABLE one. **Gate amended before any price is compared: the board must be present on a majority of days at a usable offset across at least a week. Not passed.** |
| **BH022** | **854 settled `KXMLBTOTAL` games are retrievable, not 160 — and the final total is readable off the settlement ladder with no external data.** | Kalshi `/markets?status=settled` + `/candlesticks`, 2026-08-21 | **10,431 settled rungs, 854 GAMES, 66 game-days**, back to 2026-06-30 | Mailbox 022 scoped the plan on 160 games from `record.db` (which only began 2026-08-06) and concluded a **three-week wait**. The API retains far more. Against `K014`'s bar — **481 for a large edge, 2,084 for a small one** — we are **already past the first**, 41% to the second. ⚠ **And the ladder is self-describing**: "over 10.5 yes / over 11.5 no" fixes the total at exactly 11. **104 of 104 ladders cross exactly once, none inconsistent.** Historical asks come from candlesticks (`yes_ask.close_dollars`). **One free endpoint gives the outcome, a second the price — no scraper, no paid feed, no login.** | **SETTLED.** The three-week wait does not exist. ⚠ **What has not changed: the small-edge bar needs 2,084 games at ~15/day, so a small edge stays undetectable this season** |
| **BH023** | **P1 (player props vs a sharp book) STOPS on its own pre-registered drop condition: the three de-vig methods disagree in SIGN on 8 of 14 props.** | `src/props_n3.py` → `reports/props_n3.json`; pre-registered at `PREREGISTRATION_PROPS.md` **before any number existed** | **14 props, 14 distinct players, 1 day**, both feeds pulled **12 s** apart. First open prop board in **4 days watched** | Pinnacle's prop margin **6.97 out of 100 — the fattest measured anywhere here, 3.5× its own moneyline.** Buy side **0 of 14**. Method means: proportional **+0.47¢ (57% positive)**, power **−1.50¢ (0% positive)**, Shin −0.72¢. ⚠ **The disagreement is perfectly stratified: home runs 8 of 8 disagree (ask 12–28¢, long odds); strikeouts 0 of 6 (ask 43–60¢).** Exactly the failure `PREREGISTRATION_RETAIL.md` §3a named in advance — proportional is wrong at long odds and the margin is fattest where it is worst. 4 sell-side flags: 3 are home-run props inside the disagreement; the 4th (Yamamoto) has all methods agreeing and **clears by 0.03¢**, which is a rounding error, not a finding | **DROPPED per §6.** ⚠ **And the useful half is not the null:** props ARE the first family to reach the cheap fee zone (**0.74–1.75¢, median 1.29¢**, against totals' 1.68–1.71¢) — **but the cheap rungs are cheap BECAUSE they are long odds, and long odds are exactly where de-vigging stops having one answer. The fee saving and the measurement ambiguity are the same fact.** "Go where the fee is small" is self-defeating on this route |
| **BH024** | ⚠ **CROSS-VENUE ARBITRAGE CANNOT BE MEASURED FROM THIS TAPE — 94 out of 100 apparent crossings are the recorder's own clock skew.** My first run reported **1,292 executable arbitrages, median 3.47¢, max 92¢**. It was wrong. | `src/crossvenue_arb.py` → `reports/crossvenue_arb.json`; placebo in the same file | **969 matched pairs, 202 distinct games, 27 days** (2026-08-05→08-31). Match key = same clubs + same date + **same numeric line**, three independent agreements. No doubleheaders (365 date/pair combos, all one start time). Venues track: correlation **0.871**, median gap 2.5¢ | **`cycle_id` is NOT one clock**: Kalshi and Polymarket are recorded a **median 6.5 min apart** inside one cycle (p90 8.0, max 23.8). **PLACEBO: deliberately mis-align them further and crossings scale almost perfectly linearly — 125 at 6.5 min, 274 at 19, 451 at 32, 875 at 58; correlation 0.9975, ~14.7 extra 'arbitrages' per minute of skew.** Extrapolated to zero gap: **7**, against 125 observed. Pre-game only: 125 theoretical, 44 after fees, 40 'executable' — but **84 of 100 lasted a single snapshot** and a snapshot compares prices 6.5 min apart | **NOT MEASURABLE from this instrument — which is a complete answer, not a null.** It does NOT say cross-venue arbitrage is absent; the residual is consistent with zero AND with a small effect the tape cannot resolve. **Fix is order of operations, not volume: sample both venues within seconds of each other. No extra recording at the current cadence can help** |
| **BH025** | ⚠ **POLYMARKET IS NOT FREE TO TRADE ON SPORTS, and this repo has assumed it was.** | [docs.polymarket.com/trading/fees](https://docs.polymarket.com/trading/fees), retrieved **2026-08-31** | primary source, quoted | Polymarket sports **taker = C × 0.05 × p × (1−p)**; their own words *"Makers are never charged fees. Only takers pay fees"*; worked example 100 shares at $0.50 = **$1.25**. Kalshi is `roundup(0.07 × C × P × (1−P))` — **the SAME quadratic shape at a different coefficient**. So a two-legged taker arbitrage pays **0.12 × p × (1−p) ≈ 3¢ at a 50¢ price** | **SETTLED from a primary source.** This is why the median 1¢ theoretical crossing in BH024 was never a trade |
| **BH026** | **The paired sampler removes BH024's instrument limit: the two venues are now read 84 ms apart instead of 6.5 minutes — about 4,600× tighter.** | `src/paired_sampler.py` → `data/paired.db`; `--report` runs the skew placebo every time | **66 matched pairs, all pre-game, 66 of 66 captured two-sided on BOTH venues**, 2026-08-31 20:22Z | Measured gap **median 84 ms, p90 104, max 426**, stored on every row rather than assumed. Kalshi spread median **1.0¢**. **First simultaneous reading: 0 theoretical crossings, 0 after fees on both legs** — consistent with BH024's placebo, which extrapolated to ~7 crossings across 969 pairs over 27 days. ⚠ **AND THE FIRST '0 CROSSINGS' FROM THIS SAMPLER WAS A BUG:** it captured **0 of 66 Kalshi quotes** against 66 of 66 Polymarket, because I read `orderbook`→`yes_fp` instead of `orderbook_fp`→`yes_dollars` (×100). **GUARDS #12/#23 in a script whose docstring cites them — the fifth field-name absence in three weeks.** A comparison missing one side returns 'no crossings' and is indistinguishable from a clean null; caught by counting populated columns, not by care | **APPARATUS ESTABLISHED, accumulating. One sample is NOT a result and is not reported as one.** Registered in both runner registries; own database and lock, never `record.db` |
| **BH027** | ⚠ **BH025 IS DOWNGRADED: Polymarket's sports fee cannot be verified from any free source, and C004's measured formula predates a platform migration.** | `src/poly_fee_check.py` → `reports/poly_fee_check.json` | three sources probed 2026-09-02, sports tokens taken from the live markets the paired sampler watches, so any fill would have been a sports fill by construction | Goldsky orderbook subgraph (C004's route) **HTTP 429, deprecated** — its own error: *"paused and deprecated following Polymarket's migration to V2 — the data is stale and incorrect"*. `data-api/trades` **200 but no fee field**. `clob/trades` **401**. ⚠ **So C004's 4,310-fill measurement (2026-04-20..27) PREDATES the V2 migration** — a concrete named reason it may no longer describe the venue, which is different from "the docs were wrong once" | **UNVERIFIED, both formulas.** Cost bars use the LARGER (C004, `0.10·min(p,1−p)`) because a bar should err upward; at 50¢ that is **5.00¢ against the documented 1.25¢, 4×**. Every "after fees" number computed on the documented form was too generous |
| **BH028** | ⚠ **I reimplemented the Polymarket fee in my own folder, using the weaker formula, and the repo's anti-reimplementation guard could not see it.** | `src/crossvenue_arb.py` before/after; `common/tests/test_no_fee_reimplementation.py` | — | `common/costbar.py` has implemented the Polymarket fee all along, with C004's measured form. I wrote a second one from the documentation. **The guard scans for Kalshi's `0.07`/`0.0175`; I wrote `0.05`, so it passes clean.** Now imports the shared module | **FIXED in my folder. The guard hole is REPORTED, NOT FIXED** — mailbox 025 says the guard system is being audited separately. ⚠ The hole is on the venue whose documentation has actually been proven wrong |
| **BH029** | **Kalshi ESPORTS ticker times are EASTERN, not UTC — and H10 therefore discarded the last four pre-match hours of every match.** | `src/diagnose_cross.py:event_time` (fixed); test against the recorder | **2,592 `KXCS2GAME` tickers vs 2,069 distinct Pinnacle esports start instants** | at 60 s tolerance **ET matches 2,236, UTC 1,520**; ET wins at 60/120/300 s alike. ⚠ Both counts are inflated (a ticker is matched against ANY Pinnacle start, not its own), so **the comparison is the evidence, not the levels**. `h10_passive.py:56` imports this function and skips `ts >= et`; ET is UTC−4 in August, so the computed start was **4 hours early** | **SETTLED, matching BH012's method for MLB. Direction of H10's conclusions is safe (no post-match leak) but the REGIME is not what its write-up claims** — it measured the quiet window. ⚠ Cannot be re-run here: `pyarrow` parquet is blocked by an Application Control policy |
| **BH030** | **The `*_n3` cost bar was FEE-ONLY, and it was manufacturing sell-side candidates.** | `src/costbar_local.py`; re-costed from `reports/props_n3.json` | the 4 sell-side flags in `RESULTS_PROPS_N3.md` | `bar = fee_rate_cents(ask)` charged no spread, though buying at the ask IS crossing it. With fee + half-spread: **4 flags become 1**, and the case described as clearing "by 0.03¢" now **fails by 0.46¢**. The survivor is a home-run prop already inside the 8-of-8 method disagreement | **FIXED across `props_n3`, `totals_n3`, `totals_family_n3`. No verdict moves** — a permissive bar makes a null harder to reach, so every `0 of N` stood — **but it was generating false positives on the sell side** |
| **BH031** | ⚠ **The 9-hour tape hole was NOT a missing timeout — every Polymarket call already had one. It was 328 slow requests inside it, and the watchdog cannot see a hang at all.** | `src/record.py` (leg deadline + cycle guard); mailboxes 029-031 | cycle 3719, 2026-09-07: 24 health rows against the usual 32, **zero poly rows**, so it never finished the FIRST tag | `requests`' timeout is **per socket operation, not total elapsed**. `venues.get()` retries 5× with back-off, so one call can take **165 s** legitimately; the poly leg makes up to **8 tags × 41 = 328 requests**; **328 × 165 s = 15 HOURS inside the existing timeout**, and one tag alone is 113 min. ⚠ **`runners/watchdog.ps1` restarts what is NOT RUNNING. A stalled process IS running**, so it was checked every 10 min for 9 hours and correctly called alive. Every restart mechanism here checks **liveness**; the failure was **progress** | **FIXED, two bounds, no new authority to kill anything:** a 7-min leg deadline checked BETWEEN requests (caps a leg at budget + one request, not requests × worst case), and the recorder **abandons its own cycle** after 30 min and exits so the existing watchdog restarts it. `finished_utc` stays NULL — the detector 030 identified. ⚠ The first guard's explanatory note never wrote: it reused the caller's SQLite connection, which is not thread-safe, and the bare except swallowed it — **the guard looked correct while losing the diagnosis** |
| **BH032** | **The Kalshi combo (parlay) tape is captured: 277,861 combos, 2,166,255 legs, before it ages out.** | `src/combo_recorder.py` → `data/combos.db`; registered in both runner registries | **277,861 distinct combos / 2,166,255 legs / 256,077 settled**, close times **2026-08-09 → 2026-10-18**, captured in 874 s | Cross-game `KXMVESPORTSMULTIGAMEEXTENDED` **142,400 finalized**; `KXMVECROSSCATEGORY-SHARD1` 103,200 + 21,361 active; `KXMVECROSSCATEGORY` 10,800. **NFL and NBA same-game combos return ZERO markets in every status** though 196 NBA collections exist — a collection is a template and nobody has ever requested a quote, so **"does not trade", not "data missing"**. Coherence check only, NOT a finding: payout falls **27.2 in 100 at two legs → 5.1 at eight**, smoothly, which is what multiplying probabilities must do | **CAPTURED. The factory owns the analysis.** ⚠ **93 of every 100 snapshots have bid 0 / ask 100** — combos are priced by Request For Quote, so filtering to two-sided quotes as every other recorder here does would have stored **7% of the tape and called it complete** |
| **BH033** | ⚠ **MY OWN PAGE CAP REPORTED 8,000 COMBOS WHERE THERE WERE 142,400 — 6% — and the number looked plausible.** | `src/combo_recorder.py` first run vs second | three series each returned **exactly 8,000** = 40 pages × 200 | A count equal to your own limit is never a measurement. **Third time in this folder**: BH014 (`mkts[:60]` in an undocumented order starved 40 markets a cycle) and the blind-spot census that returned zero baseball. Also fixed: a single timed-out page silently ended that series' sweep, so every run would truncate at a different random point while reporting a clean total | **FIXED** — the cursor runs to exhaustion bounded by a wall-clock deadline (which cannot truncate silently), and a dropped page retries the same cursor, with repeated failure flagged on the sweep row |
...
| **F002** | ⚠ **PARTLY RETRACTED 2026-08-19 — MY CORRECTION WAS ITSELF WRONG, AND SO WAS MY DIAGNOSIS OF WHY.** The plan's number IS wrong, but by less than I said, and not for the reason I gave. **Exact, agreed with `coordinator` after both of us erred once: one zero-skill strategy reaches +30% over 100 bets 1 time in 4,893, and the best of 2,000 reaches it 34 times in 100.** My figures were 1 in 2,289 and 58 in 100. ~~The best-of-N table understates the danger by about four times.~~ | strategy-factory | Exact binomial tail, no simulation; one observation is one 100-bet strategy at 50c paying the real entry fee from `common/kalshi_fees.py`. **The simulation is now known to be incapable of resolving this tail** — it expects ~4 hits in 20,000 runs — and the script prints its hit count for that reason | **The error is the DENOMINATOR, not the fee.** Buying at 50c takes **52c** out of the account — price *and* fee — so +30% means turning 52c into 67.6c, which needs **68** wins of 100, not 67. One win of difference halves the answer: **1 in 4,893** against my 1 in 2,289 and the plan's 1 in 10,000. Verified at 1, 5, 20 and 100 contracts per order; per-order fee rounding does not move the threshold. ⚠ **My stated cause — that the plan charged the fee twice — is WITHDRAWN.** Charging it twice gives 1 in 10,920, which is close to the plan's figure, and that near-match is exactly why I believed it. The plan's number was a Monte Carlo estimate off **two hits in 20,000 runs** | **SETTLED by arithmetic at the third attempt, and the lesson is about method, not about the number.** The "typical best" column always reproduced (10.0 vs 10.1 · 18.0 vs 17.9 · 26.0 vs 25.6 · 30.0 vs 29.5); only the tail column was ever in dispute. **Three findings survive.** (1) The plan's 1-in-10,000 was wrong and the user's own claim — that a single pre-specified strategy at +30% is genuinely rare — is *correct at the true figure of 1 in 4,893*. (2) The danger is entirely in the selection: 34 in 100 at best-of-2,000. (3) **A tail probability read off a simulation needs its hit count printed beside it**, which is how both wrong versions survived being written down. Reproduce: `py -3 strategy-factory/src/bestofn.py` |
