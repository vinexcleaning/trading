...
| C001 | Kalshi `greater` ladders are monotone in strike — no arbitrage. | crypto | `src/ladder_arb.py` → `reports/ladder_arb.json` | 3,187 scans / 26 events / 10.5 min | 08-01 | **0 violations**, gross or net | n/a | — | **SUGGESTIVE** (clean null but only 10.5 min). ⚠ The "prior 1,083-scan null" this row cited is **[K007](#section-6--kalshi-market-scan-exchange-wide-screen-weather-flow-sports)**, which found **52 violations, not zero** — citation corrected in `crypto/` 2026-08-03 |
