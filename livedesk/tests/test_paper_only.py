"""The guard that makes "this window cannot send an order" structural.

Copied unchanged in mechanism from `mlb-paper/tests/test_paper_only.py`, as
the mailbox instruction required -- deliberately NOT a third style of the same
test. Walks every .py in this package and fails the build if order-shaped
code, credential reads, or non-GET HTTP appear anywhere. Includes a
**guard-rot check** (GUARDS #9): the detector is run against a deliberately
planted violation, so a detector that has silently stopped detecting fails too.
