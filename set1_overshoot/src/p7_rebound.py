"""Tennis rebound study -- ADOPTED from `coordinator/studies/rebound2.py`
2026-08-26 per `tennis` mailbox 019, plus the spread gate that message asked for.

WHAT WAS ADDED HERE, AND WHY IT IS THE WHOLE POINT OF THE RE-RUN
----------------------------------------------------------------
019 found a contradiction it could not explain: on the main tour the DEEP-dip
version of the hypothesis is flat, while on ITF it clears Benjamini-Hochberg
with a +5 to +6 point lift. Its own recommended next experiment:

  "Re-run the ITF deep-dip cells restricted to spreads of 3 cents or less. If
   the lift survives a tight-spread filter it is real and ITF is genuinely
   slower. If it vanishes, it was the mid all along."

`--max-spread` does that. **It gates WHERE A MEASUREMENT MAY BE TAKEN, not
which candles exist.** Deleting wide candles from the path would change the
running peak and the future maximum -- i.e. it would change what the price did,
not just where we looked at it. The gate is applied identically to treated
events and to control minutes, because gating only the treatment would replace
one confound with a worse one.
...
MIN_CELL = 30
CUTOFF = "2026-08-01"
MAX_SPREAD_KEEP = 25   # path-level sanity, unchanged from v2
...
def paths(conn, series_filter):
    ...
    for r in cur:
        ...
        if t in meta:
            b, a = r["bid_c"], r["ask_c"]
            if 0 < b <= a < 100 and (a - b) <= MAX_SPREAD_KEEP:
                buf.append((b, a, (b + a) / 2.0))
    if t0 in meta and len(buf) >= 30:
        yield t0, meta[t0], buf


def main(series_filter, out_path, label, max_spread=99, strict=False):
    ...
    for tk, m, path in paths(conn, series_filter):
        ...
        for i, (bid, ask, mid) in enumerate(path):
            # the gate: may we MEASURE here? The path itself is untouched,
            # so `peak` and `fmax` still describe what the price actually did.
            ...
            for P in PEAKS:
                if not tight:
                    if not strict:
                        continue
                    # STRICT MODE. Plain `--max-spread` turned out to be a
                    # weaker test than it looks: skipping a wide minute does not
                    # DROP the event, it relocates it to the next tight minute,
                    # so the event count barely moves (268 -> 259) even though
                    # HALF of ITF minutes near 30c are wider than 3c. Same
                    # events, later timestamps.
                    #
                    # Strict mode asks the harder question 019 was really
                    # after: does the dip only ever qualify while the book is
                    # wide? If a cell first qualifies at a wide minute it is
                    # burned for this ticker and can never fire later.
                    if peak >= P:
    ...
    print(f"    measurements gated to spreads <= {max_spread}c "
          f"(applied to events AND controls alike)"
          + ("  [STRICT: wide-qualifying events DROPPED]" if strict else ""))
...
if __name__ == "__main__":
    ...
    ap.add_argument("--max-spread", type=int, default=99,
                    help="only measure at candles this tight; 019 asks for 3")
    ap.add_argument("--strict", action="store_true",
                    help="DROP an event whose dip first qualified while wide, "
                         "instead of relocating it to the next tight minute")
    a = ap.parse_args()
    main(a.series, a.out, a.label, a.max_spread, a.strict)
